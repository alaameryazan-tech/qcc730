...
//	uint8_t last_packet = 0;
	...
	while (1)
	{
		switch (state)
		{
			...
			case REQUEST_ELF_HEADER:
				{
					uint32_t running_offset = 0, prog_len;
					uint8_t soft_id = 0, bitmap;
					prog_len = sizeof(Elf32_Ehdr);
					bitmap = sys_loader_cntxt.softid_dfu_bitmap ^ sys_loader_cntxt.softid_dfu_done_bitmap;
					...
					if(bitmap & SBL_ID)
					{
						...
						soft_id = SBL_ID;
						p_sbl_desc->dfu_status = DFU_STARTED_FROM_SBL;
						state = SEND_REBOOT_CMD;
						break;
					}
					else if(bitmap & WLAN_IMG_ID)
					{
						soft_id = WLAN_IMG_ID;
					}
					else if(bitmap & BDF_ID)
					{
						soft_id = BDF_ID;
					}
					else if(bitmap & REGDB_ID)
					{
						soft_id = REGDB_ID;
					}
                    else if(bitmap & WLAN_INI)
					{
						soft_id = WLAN_INI;
					}
					else if(bitmap == 0)
					...
					{
						sbl_printf("SBL ERROR: unknown resource ID\r\n");
					}

					sys_loader_cntxt.start_offset = 0;
					sys_loader_cntxt.running_offset = running_offset;
					sys_loader_cntxt.softid_in_progress = soft_id;
					sys_loader_cntxt.prog_len = prog_len;

					if(send_dfu_data_transfer_req(running_offset, prog_len, soft_id))
					{

						state = WAIT_FOR_ELF_HEADER;
					}
				}
				break;

			case WAIT_FOR_ELF_HEADER:
				{
					ring_data_desc_t ring_data_desc;
					if(recieve_dfu_data_transfer_confirm(&ring_data_desc))
					{
						if(ring_data_desc.data_len == sizeof(Elf32_Ehdr))
						{
							Elf32_Ehdr* eh = (Elf32_Ehdr*)(ring_data_desc.p_buff);
							sys_loader_cntxt.start_offset = eh->e_phoff;
							sys_loader_cntxt.running_offset = eh->e_phoff;
							sys_loader_cntxt.num_of_ph = eh->e_phnum;
							sys_loader_cntxt.prog_len = (eh->e_phnum)*sizeof(Elf32_Phdr);
							state = REQUEST_PROGRAM_HEADER;
						}
						else
						{
							state = REQUEST_ELF_HEADER;
			...
			case REQUEST_PROGRAM_HEADER:
				{
					uint32_t running_offset, prog_len;
					uint8_t soft_id;
					running_offset = sys_loader_cntxt.running_offset;
					soft_id = sys_loader_cntxt.softid_in_progress;
					prog_len = sys_loader_cntxt.prog_len;
					if(send_dfu_data_transfer_req(running_offset, prog_len, soft_id))
						state = WAIT_FOR_PROGRAM_HEADER;
				}
				break;

			case WAIT_FOR_PROGRAM_HEADER:
				{
					ring_data_desc_t ring_data_desc;
					memset(&ring_data_desc,0,sizeof(ring_data_desc_t));
					if(recieve_dfu_data_transfer_confirm(&ring_data_desc))
					{
						if(ring_data_desc.data_len == sizeof(Elf32_Phdr))
						{
							Elf32_Phdr* ph_table = (Elf32_Phdr*)(ring_data_desc.p_buff);

							sbl_printf("========================================");
							sbl_printf("========================================\r\n");
							sbl_printf("idx Type       offset   vaddr      paddr      "
									"filesize memsize flag     align \r\n");
							sbl_printf("========================================");
							sbl_printf("========================================\r\n");

							for(int i=0; i < sys_loader_cntxt.num_of_ph; i++) {
								sbl_printf(" %d  ", i);
								sbl_printf("0x%08x ", (unsigned int)ph_table[i].p_type);
								sbl_printf("0x%06x ", (unsigned int)ph_table[i].p_offset);
								sbl_printf("0x%08x ", (unsigned int)ph_table[i].p_vaddr);
								sbl_printf("0x%08x ", (unsigned int)ph_table[i].p_paddr);
								sbl_printf("0x%05x ", (unsigned int)ph_table[i].p_filesz);
								sbl_printf("0x%05x ", (unsigned int)ph_table[i].p_memsz);
								sbl_printf("0x%08x ", (unsigned int)ph_table[i].p_flags);
								sbl_printf("0x%05x", (unsigned int)ph_table[i].p_align);
								sbl_printf("\r\n");
							}
							sbl_printf("========================================");
							sbl_printf("========================================\r\n");
							...
							sys_loader_cntxt.start_offset = ph_table[0].p_offset;
							sys_loader_cntxt.running_offset = ph_table[0].p_offset;
							sys_loader_cntxt.file_size = ph_table[0].p_filesz;
							sys_loader_cntxt.phy_addr = ph_table[0].p_paddr;
							sys_loader_cntxt.end_offset = ph_table[0].p_offset + ph_table[0].p_filesz;
							sys_loader_cntxt.total_pkts = (ph_table[0].p_filesz)/DFU_BUFF_SIZE;
							sys_loader_cntxt.curr_pkt_count = 0;
							uint32_t remain_len;
							remain_len = (ph_table[0].p_filesz)%DFU_BUFF_SIZE;
							if(remain_len)
							{
								sys_loader_cntxt.total_pkts += 1;
							}
							sbl_printf("SBL LOG: remain_len=%d, total_pkts=%d\r\n",(int)remain_len,(int)sys_loader_cntxt.total_pkts);
							attach_buffer_to_element(A2F_RING, ring_data_desc.index);
			...
			case WAIT_FOR_PROGRAM:
				{
					...
					if(recieve_dfu_data_transfer_confirm(&ring_data_desc))
					{
						...
						sbl_printf("SBL LOG: dst=%x, src=%x, length=%x \r\n",(unsigned int)dst,(unsigned int)src,(unsigned int)length);
						int rram_write_status;
						rram_write_status = (nt_bl_rram_write(dst, src, length));
			...
			case RESOURCE_TRANSFER_CONFIRM:
				{
					...
					{
						sbl_printf("SBL: LOG : recieved transfer confirm \r\n");
						if(WLAN_IMG_ID == soft_id)
							p_ferm_fdt->wlan_img.dfu_status = DFU_FINISHED;
						else if(BDF_ID == soft_id)
							p_ferm_fdt->bdf.dfu_status = DFU_FINISHED;
						else if(REGDB_ID == soft_id)
							p_ferm_fdt->reg_db.dfu_status = DFU_FINISHED;
						else if(WLAN_INI == soft_id)
							p_ferm_fdt->wlan_ini.dfu_status = DFU_FINISHED;
