...
void sigma_print_test_results(SIGMA_CXT *p_tCxt, SIGMA_STATS *pktStats)
{
    ...
    SIGMA_PRINTF("\t%u KBytes %u bytes (%u bytes) in %u seconds %u ms (%u miliseconds)\n",
                 (uint32_t)(total_bytes / 1024), (uint32_t)(total_bytes % 1024), (uint32_t)total_bytes, sec_interval,
                 (uint32_t)(total_interval % 1000), total_interval);
...
void send_ack(SIGMA_CXT *p_tCxt, struct sockaddr *faddr, int addrlen)
{
    ...
    while (retry) {
        ...
        if (send_result < 0) {
            ...
            do {
                received = recvfrom(p_tCxt->sock_local, (char *)(&p_tCxt->buffer[0]), CFG_PACKET_SIZE_MAX_RX,
        ...
        } else /* Sending ACK is successful.  Now waiting 1000 ms for more endmark packets from client. */
        {
            ...
            if (conn_sock > 0) {
                {
                    received = recvfrom(p_tCxt->sock_local, (char *)(&p_tCxt->buffer[0]), CFG_PACKET_SIZE_MAX_RX, 0,
                                        NULL, NULL);
                }
...
const QAPI_Console_Command_Group_t wificert_shell_cmd_group = {
    WiFiCERT_SHELL_GROUP_NAME, sizeof(wificert_shell_cmds) / sizeof(QAPI_Console_Command_t), wificert_shell_cmds};
