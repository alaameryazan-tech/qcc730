...
void sigma_udp_rx(void *sigma_context)
{
    ...
    while (!sigma_rx_quit) /* Main loop */
    {
        ...
        while (!sigma_rx_quit) /* Receive loop */
        {
            ...
            if (received >= 0) {
                if (received != sizeof(EOT_PACKET) || ((EOT_PACKET *)p_tCxt->buffer)->code != (int)END_OF_TEST_CODE) {
    ...
    ERROR_3:
        ...
        if (address_str) {
            SIGMA_PRINTF("Received %u bytes, Packets %u from %s:%u\n", (uint32_t)p_tCxt->pktStats.bytes,
                         p_tCxt->pktStats.pkts_recvd, *address_str, ntohs(((struct sockaddr_in *)from)->sin_port));
        } else {
            SIGMA_PRINTF("Received %u bytes, Packets %u from %s:%u\n", (uint32_t)p_tCxt->pktStats.bytes,
                         p_tCxt->pktStats.pkts_recvd, "0x0", ntohs(((struct sockaddr_in *)from)->sin_port));
        }
...
void sigma_udp_tx(void *sigma_context)
{
    ...
    SIGMA_PRINTF("\nSent %u packets, %u bytes to %s %u (%u)\n", cur_packet_number, (uint32_t)p_tCxt->pktStats.bytes,
                 ip_str, p_tCxt->params.tx_params.port, cur_packet_number - n_send_ok);
