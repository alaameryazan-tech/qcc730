...
@brief This section provides APIs, macros definitions, enumerations and data structures
...
/**
This event ID indicates to the application that authentication is pending for a go negotiation request received from a
peer device. An object of structure qapi_WLAN_P2P_Req_To_Auth_Event_t is used to pass the event information to the
application. Application should decide to authenticate or reject the peer for a connection and call qapi_WLAN_P2P_Auth()
for the decided action. Alternately, the application can call qapi_WLAN_P2P_Auth() to authenticate the peer before group
owner negotiation starts.
...
/**
This event ID indicates to the application that a group owner negotiation result is received from the target. An object
of type #qapi_WLAN_P2P_Go_Neg_Result_Event_t is used to pass the event information to the application.\n Event handling
process: \n (1) Depending on the device's role through negotiation, use one of the following: \n (a) If the device
becomes the group owner through negotiation, use qapi_WLAN_P2P_Start_Go() to start the group owner operation. \n (b)
Else, if the device becomes a group client, call qapi_WLAN_Set_Param() to set the group SSID parameter using following
...
/**
This event ID indicates reception of an invitation request from a peer device. An object of struct
qapi_WLAN_P2P_Invite_Req_Event_t is used to pass the event information to the application. Use
qapi_WLAN_P2P_Invite_Auth() API to authenticate or reject this request. Application should check if the group is one of
the stored persistent groups before deciding authenticate/reject action.
...
/**
This event indicates that an invitation response was sent after receiving invitation request from a peer device. An
object of structure qapi_WLAN_P2P_Invite_Rcvd_Result_Event_t is used to pass the event information to the application.
If the event indicates success and if the device's role is group owner then it can start group owner operation at this
point by calling qapi_WLAN_P2P_Start_Go().
...
/**
This event indicates that invitation request was sent to a peer device and a response has been received. An object of
structure qapi_WLAN_P2P_Invite_Sent_Result_Event_t is used to pass the event information to the application. If the
event indicates success and if the device's role is group owner then it can start group owner operation at this point by
calling qapi_WLAN_P2P_Start_Go().
...
This feature applies only to P2P group owners. If enabled, NOA attribute is added in the beacons, probe responses or
sent as action frames. If the notice of absence is disabled, NOA attributes are omitted from the above mentioned frames.
\n Maximum 4 NOA descriptors are allowed on a session.\n To enable NOA, an object of structure
qapi_WLAN_P2P_Noa_Descriptor_Params_tis should be used to call qapi_WLAN_Set_Param() with following arguments,\n
    device_ID: 0 (P2P mode is allowed only for device 0) \n
    group_ID: #__QAPI_WLAN_PARAM_GROUP_P2P \n
...
@dependencies
P2P find should be used to get a list of peer devices before using this structure. Each entry in the list will be of
type qapi_WLAN_P2P_Device_Lite_t. Refer to qapi_WLAN_P2P_Node_List_Params_t for more information.
...
typedef enum {
    ...
    QAPI_WLAN_P2P_DISC_ONLY_SOCIAL_E =
        1, /**< Scan only social wireless channels (i.e., channels 1, 6, 11 in 2.4 GHz band). */
...
Applications can use this data structure or each of the union members individually to configure device parameters by
calling qapi_WLAN_Set_Param() with following arguments,\n device_ID: 0 (P2P mode is allowed only for device 0) \n
       group_ID: #__QAPI_WLAN_PARAM_GROUP_P2P \n
...
__QAPI_WLAN_P2P_GO_NEG_RESULT_EVENTID event. If the device becomes the group owner, qapi_WLAN_P2P_Start_Go() should be
invoked to start the group owner operation.\n If the user has no preference over GO's operating channel,
__QAPI_WLAN_P2P_AUTO_CHANNEL should be used for the argument 'channel'.
