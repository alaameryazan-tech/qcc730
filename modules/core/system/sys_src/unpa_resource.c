...
void unpa_update_resource(unpa_client *client)
{
    ...
    if (unpa_resource_has_attribute(resource, UNPA_RESOURCE_DRIVER_UNCONDITIONAL) ||
        active_agg != resource->active_state || old_sleep_state != resource->sleep_state) {
...
{
    if (update_fcn == unpa_min_update_fcn || update_fcn == unpa_max_update_fcn ||
...
unpa_resource_state unpa_min_update_fcn(unpa_resource *resource, unpa_client *client)
{
    ...
    if (resource->agg_state[UNPA_SLEEP_INDEX] == 0) {
        ...
        resource->sleep_state = MIN(resource->agg_state[UNPA_REQUIRED_INDEX], resource->agg_state[UNPA_SLEEP_INDEX]);
    }
    ...
    return MIN(resource->agg_state[UNPA_REQUIRED_INDEX], resource->agg_state[UNPA_SUPPRESSIBLE_INDEX]);
}
...
unpa_resource_state unpa_max_update_fcn(unpa_resource *resource, unpa_client *client)
{
    ...
    /* Return active_agg */
    return MAX(resource->agg_state[UNPA_REQUIRED_INDEX], resource->agg_state[UNPA_SUPPRESSIBLE_INDEX]);
}
...
unpa_resource_state unpa_binary_update_fcn(unpa_resource *resource, unpa_client *client)
{
    ...
    /* Compute sleep_state */
    resource->sleep_state = (MAX(resource->agg_state[UNPA_REQUIRED_INDEX], resource->agg_state[UNPA_SLEEP_INDEX]) > 0);

    /* Return active_agg */
    return (MAX(resource->agg_state[UNPA_REQUIRED_INDEX], resource->agg_state[UNPA_SUPPRESSIBLE_INDEX]) > 0);
}
