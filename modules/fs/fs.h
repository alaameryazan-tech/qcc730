 ...
 * @retval <0 a negative errno code on error.
 */
int vfs_opendir(struct fs_dir_t *zdp, const char *path);
 ...
 */
int vfs_stat(const char *path, struct fs_dirent *entry);
 ...
 * @retval <0 an other negative errno code on error.
 */
int vfs_statvfs(const char *path, struct fs_statvfs *stat);
 ...
 * @retval <0 negative errno code on error.
 */
int vfs_mkfs(int fs_type, uintptr_t dev_id, void *cfg, int flags);
