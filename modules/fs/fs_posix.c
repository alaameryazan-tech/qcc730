...
static struct posix_fs_desc *posix_fs_alloc_obj(char is_dir)
{
    int i;
    struct posix_fs_desc *ptr = NULL;
    // unsigned int key = irq_lock(); // taskENTER_CRITICAL();?

    for (i = 0; i < CONFIG_POSIX_MAX_OPEN_FILES; i++) {
        if (desc_array[i].used == 0) {
            ptr = &desc_array[i];
            ptr->used = 1;
            ptr->is_dir = is_dir;
            break;
    ...
    // irq_unlock(key); //taskEXIT_CRITICAL();
...
int _open(const char *name, int flags, ...)
{
    ...
    fd = z_reserve_fd();
    if (fd < 0) {
        printf("z_reserve_fd failed.\n");
        return -1;
    }

    ptr = posix_fs_alloc_obj(0);
    if (ptr == NULL) {
        printf("posix_fs_alloc_obj failed.\n");
        z_free_fd(fd);
        errno = EMFILE;
        return -1;
    }
    ...
    rc = vfs_open(&ptr->file, name, zmode);
...
static int fs_ioctl_vmeth(void *obj, unsigned int request, va_list args)
{
    ...
    switch (request) {
        case ZFD_IOCTL_LSEEK: {
            ...
            rc = vfs_seek(&ptr->file, offset, whence);
            if (rc == 0) {
                rc = vfs_tell(&ptr->file);
            }
            break;
        }
...
static ssize_t fs_write_vmeth(void *obj, const void *buffer, size_t count)
{
    ...
    rc = vfs_write(&ptr->file, buffer, count);
    if (rc < 0) {
        errno = -rc;
        return errno;
    }
...
static ssize_t fs_read_vmeth(void *obj, void *buffer, size_t count)
{
    ...
    rc = vfs_read(&ptr->file, buffer, count);
    if (rc < 0) {
        errno = -rc;
        return errno;
    }
...
DIR *opendir(const char *dirname)
{
    ...
    rc = vfs_opendir(&ptr->dir, dirname);
    if (rc < 0) {
        posix_fs_free_obj(ptr);
        errno = -rc;
        return NULL;
...
struct dirent *readdir(DIR *dirp)
{
    ...
    rc = vfs_readdir(&ptr->dir, &fdirent);
    if (rc < 0) {
        errno = -rc;
        return NULL;
    }
...
int stat(const char *path, struct stat *buf)
{
    ...
    rc = vfs_statvfs(path, &stat_vfs);
    if (rc < 0) {
        errno = -rc;
        return errno;
    }

    rc = vfs_stat(path, &stat_file);
    if (rc < 0) {
        errno = -rc;
        return errno;
    }
