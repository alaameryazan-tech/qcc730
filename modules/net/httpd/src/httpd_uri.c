...
static httpd_uri_t *httpd_find_uri_handler(struct httpd_data *hd, const char *uri, size_t uri_len,
                                           httpd_method_t method, httpd_err_code_t *err)
{
    if (err) {
    ...
    for (int i = 0; i < hd->config.max_uri_handlers; i++) {
        ...
        if (hd->config.uri_match_fn ? hd->config.uri_match_fn(hd->hd_calls[i]->uri, uri, uri_len)
...
esp_err_t httpd_register_uri_handler(httpd_handle_t handle, const httpd_uri_t *uri_handler)
{
    ...
    if (httpd_find_uri_handler(handle, uri_handler->uri, strlen(uri_handler->uri), uri_handler->method, NULL) != NULL) {
        ESP_LOGW(TAG, LOG_FMT("handler %s with method %d already registered"), uri_handler->uri, uri_handler->method);
...
esp_err_t httpd_uri(struct httpd_data *hd)
{
    ...
    if (res->field_set & (1 << UF_PATH)) {
        uri = httpd_find_uri_handler(hd, req->uri + res->field_data[UF_PATH].off, res->field_data[UF_PATH].len,
                                     req->method, &err);
    }
