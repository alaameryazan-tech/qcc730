...
*/
int32_t QAT_Memcmpi(const void *Source1, const void *Source2, uint32_t Size);
...
*/
qbool_t QAT_String_To_Integer(const char *String, int32_t *Output);
...
/**
   ...
   @return
    ...
    - false if there was an error.
*/
qbool_t QAT_Hex_String_To_ULL(char *String, uint64_t *Output);
...
/**
   ...
   @param[in] QAT_Handle   Handle of the QAT group that is displaying the
                            message.
   @param[in] Function_Name Name of the function to be displayed.
   @param[in] Result        Return code from the function call.
*/
void QAT_Display_Function_Status(char *Function_Name, qapi_Status_t Result);
...
*/
void QAT_Dump_Data(const char *Prefix, uint16_t Length, const uint8_t *Buffer);
