...
#ifdef MBEDTLS_MD4_C
/**
 * md4_vector_new - MD4 hash for data vector
 ...
 * Returns: 0 on success, -1 on failure
 */
int md4_vector_new(size_t num_elem, const uint8_t *addr[], const size_t *len, uint8_t *mac);
#endif
 ...
 */
int __must_check fips186_2_prf(const uint8_t *seed, size_t seed_len, uint8_t *x, size_t xlen);
#endif
...
/**
 * aes_encrypt_deinit_new - Deinitialize AES encryption
 * @ctx: Context pointer from aes_encrypt_init()
 */
void aes_encrypt_deinit(void *ctx);

/**
 * aes_decrypt_init_new - Initialize AES for decryption
 * @key: Decryption key
 * @len: Key length in bytes (usually 16, i.e., 128 bits)
 * Returns: Pointer to context data or %NULL on failure
 */
 ...
 */
int crypto_ec_get_publickey_buf(struct crypto_key *key, uint8_t *key_buf, int len);
 ...
 * Returns: Crypto key
 */
struct crypto_key *crypto_ec_get_key(const uint8_t *privkey, size_t privkey_len);
