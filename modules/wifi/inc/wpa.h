...
struct wpa_sm_ctx {
    ...
    int (*set_key)(void *ctx, wpa_alg alg, const uint8_t *addr, int key_idx, int set_tx, const uint8_t *seq,
                   size_t seq_len, const uint8_t *key, size_t key_len);
    void (*scan)(void *eloop_ctx, void *timeout_ctx);
    struct wpa_ssid *(*get_ssid)(void *ctx);
    int (*get_bssid)(void *ctx, uint8_t *bssid);
    int (*ether_send)(void *ctx, const uint8_t *dest, uint16_t proto, const uint8_t *buf, size_t len);
    int (*get_beacon_ie)(void *ctx);
    void (*cancel_auth_timeout)(void *ctx);
    uint8_t *(*alloc_eapol)(void *ctx, uint8_t type, const void *data, uint16_t data_len, size_t *msg_len,
                            void **data_pos);
    int (*add_pmkid)(void *ctx, const uint8_t *bssid, const uint8_t *pmkid);
    int (*remove_pmkid)(void *ctx, const uint8_t *bssid, const uint8_t *pmkid);
...
int wpa_sm_set_assoc_wpa_ie_default(struct wpa_sm *sm, uint8_t *wpa_ie, size_t *wpa_ie_len);
...
int wpa_sm_rx_eapol(struct wpa_sm *sm, const uint8_t *src_addr, const uint8_t *buf, size_t len);
...
static inline void wpa_sm_set_pmk(struct wpa_sm *sm, const uint8_t *pmk, size_t pmk_len) {}
...
static inline void wpa_sm_set_config(struct wpa_sm *sm, struct wpa_ssid *config) {}
...
static inline int wpa_sm_set_assoc_wpa_ie(struct wpa_sm *sm, const uint8_t *ie, size_t len)
{
    return -1;
}

static inline int wpa_sm_set_assoc_wpa_ie_default(struct wpa_sm *sm, uint8_t *wpa_ie, size_t *wpa_ie_len)
{
    return -1;
}

static inline int wpa_sm_set_ap_wpa_ie(struct wpa_sm *sm, const uint8_t *ie, size_t len)
{
    return -1;
}

static inline int wpa_sm_set_ap_rsn_ie(struct wpa_sm *sm, const uint8_t *ie, size_t len)
{
    return -1;
}
...
static inline unsigned int wpa_sm_get_param(struct wpa_sm *sm, enum wpa_sm_conf_params param)
{
    return 0;
}
...
static inline int wpa_parse_wpa_ie(const uint8_t *wpa_ie, size_t wpa_ie_len, struct wpa_ie_data *data)
{
    return -1;
}
...
static inline int wpa_sm_rx_eapol(struct wpa_sm *sm, const uint8_t *src_addr, const uint8_t *buf, size_t len)
{
    return -1;
}
