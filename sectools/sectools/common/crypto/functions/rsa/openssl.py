...
class RsaOpenSSLImpl(RsaBase):
    ...
    def check_cert_leading_zero(self, certificate, algo, ignore_retcode=False):
        if algo != 'sha256':  #only support sha256WithRSAEncryption algorithm
            return 0
        cert_path = c_path.create_tmp_file(certificate)
        sig_opt = ['-text', '-noout', '-certopt', 'ca_default', '-certopt', 'no_validity', '-certopt', 'no_serial', '-certopt', 'no_subject', '-certopt', 'no_extensions', '-certopt', 'no_signame']
        sig_pattern = r"Signature Algorithm: sha256WithRSAEncryption\n         00"
        sig_cmd = ([self.openssl, 'x509'] + ['-in', cert_path] + sig_opt)
        try:
            sig = run_command(sig_cmd, ignore_retcode=ignore_retcode)
            leading_zero = re.search(sig_pattern, sig)
            if leading_zero:
                return 1
            else:
                return 0
