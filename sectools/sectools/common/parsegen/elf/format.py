...
class ParseGenElf(object):
    ...
    def __init__(self, data=None,
        ...
        self.has_sections = getattr(elf_properties, "has_sections", False)
        if elf_properties and not self.has_sections:
    ...
    def shift(self, offset, size, align=None, after=False):
        ...
        min_offset = None
        least_common_alignment_multiple = 0x1
        hdrs = self.phdrs + self.non_encapsulated_sections
        for i_hdr in hdrs:
            current_alignment = i_hdr.alignment
            if current_alignment and least_common_alignment_multiple % current_alignment:
                least_common_alignment_multiple = least_common_alignment_multiple * current_alignment // \
                                                  gcd(least_common_alignment_multiple, current_alignment)
        ...
        if min_offset is not None:
            ...
            # Update the shift amount when image_type's <has_sections> is true
            if self.has_sections and least_common_alignment_multiple > shift:
                shift += least_common_alignment_multiple - shift
            for i_hdr in hdrs:
        ...
        if min_offset is not None and self.ehdr.e_shnum:
            if not after or self.ehdr.e_shoff + self.ehdr.e_shentsize * self.ehdr.e_shnum > offset:
                shift = offset + size - min_offset
                # Update the shift amount when image_type's <has_sections> is true
                if least_common_alignment_multiple > shift:
                    shift += least_common_alignment_multiple - shift
