...
class HTTPClient(BaseClient):
    ...
    def query_server(url, data=None, cert_file=None):
        ...
        try:
            ...
            if sys.version_info < (2, 7, 9):
                if cert_file and "https://" in url:
                    raise RuntimeError("Python version 2.7.9 or later is required to establish HTTPS connection with server.")
                response = urlopen(req)
            else:
                try:
                    # /etc/ssl/certs is included here as a backup certificate location for when qti_ca_file.pem
                    # expires or otherwise becomes invalid yet the client machine has the necessary certificate
                    # already installed. Necessary for QTI remote signing via an HTTPS-enabled server.
                    response = urlopen(req, cafile=cert_file, capath="/etc/ssl/certs")
                except TypeError:
