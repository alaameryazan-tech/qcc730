...
def PrintConfig(ModName, CONFIG, devcfgdatafilename, ConfigType ):
   ...
   CONFIG.write ("\n const void * " + dalprop_structptrs_str + "["  + "] =  {\n")
   for struct_ptrs in StructPtrs:
      local_struct =0
      for struct in StructDef:
         name = StructDef[struct][0]
   ...
   CONFIG_DATA.write ("/** System Generated File \n")
   CONFIG_DATA.write ("  * Cann't Change Manually */ \n\n")
   CONFIG_DATA.write ("#include \"nt_devcfg_types.h\" \n")
   CONFIG_DATA.write ("#include \"nt_devcfg_def.h\" \n")
   #CONFIG_DATA.write ("#include \"nt_devcfg_parser.c\" \n")
   CONFIG_DATA.write ("#include <stddef.h> \n") #include <stddef.h> for null
   CONFIG_DATA.write ("\nextern const void * " + dalprop_structptrs_str + "[];\n")
   CONFIG_DATA.write ("\nextern const uint32 " + dalprop_propbin_str + "[];\n")
   #CONFIG_DATA.write ("\nuint32 i = sizeof(" + dalprop_propbin_str + ")/sizeof(" + dalprop_propbin_str + "[0]);\n")
   #CONFIG_DATA.write ("\nint nt_devcfg_parse( uint32 " + dalprop_propbin_str + " );\n")
   ...
   if len(ConfigStructList)!=0:
      ...
      CONFIG.write("\n\n\nt StringDevice " + driver_list_str + "[] = ")
      CONFIG.write("{\n\t\t\t")
...
def PrintModDir(ModDirFileName, DriverList):
   ...
   MODDIR.write ("#include \"nt_devcfg_def.h\" \n")
   MODDIR.write ("#include \"DALReg.h\" \n\n")
   MODDIR.write ("#include\"nt_devcfg_types.h\"\n")
...
def main():
   ...
   if options.xml_filename is None:
    ...
    xml_file = options.xml_filename #"DevCfg_master_devcfg_out.xml"#

   if options.config_type is not None:
      ConfigType = options.config_type #"DevCfg_master_devcfg_out.xml"#
   if options.output_dir is not None:
      DirName = options.output_dir #"export"#
   if options.mod_dir_filename is not None:
      ModDirFileName = options.mod_dir_filename #"DevCfg_master_fom_out.xml"#
   if options.cfg_dir_filename is not None:
      ConfigFileName = options.cfg_dir_filename #"data.h"#
   if options.devcfgdata_filename is not None:
      DevcfgDataFileName = options.devcfgdata_filename #"devcfg_devcfg_data.c"#
   ...
   if ConfigFileName != None:
      CONFIG = open(ConfigFileName, 'w')   #### output to DALConfig_devcfg.c(config file)
      CONFIG.write ("/** System Generated File \n")
      CONFIG.write ("  * Cann't Change Manually */ \n\n")
      CONFIG.write ("#include \"nt_devcfg_def.h\" \n")
      CONFIG.write ("#include \"nt_devcfg_types.h\" \n\n")
      #CONFIG.write ("#include <stddef.h> \n") #include <stddef.h> for null
