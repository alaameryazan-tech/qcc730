...
class Firmware_Descriptor_Entry:
    ...
    def from_xml (self, xml_root):
        ...
        if 'dirname' in xml_root.attrib:
            self.dirname = xml_root.attrib['dirname']
...
class Firmware_Descriptor_Table:
    ...
    def from_binary (self, data):
        ...
        self.version = all_bytes[4]
        self.rank = all_bytes[8]
        self.status = all_bytes[12]
        self.num_images = all_bytes[13]
        self.reserved = str(all_bytes[14])
    ...
    def from_xml (self, xml_root):
        ...
        look like:
        <table>
        <header rank="0" signature="0x54445746" status="1" version="1" load_type="0"/>
...
class Firmware_Descriptor:
    ...
    def gen_xml_program (self, filename, table_filepath, table_index = None):
        ...
        def add_basic_xml_attributes (xml_elem, operation, dirname, filename, start_sect, num_sect):
        ...
        table_dirname = ntpath.dirname(table_filepath)
        table_filename = ntpath.basename(table_filepath)
        add_basic_xml_attributes (xml_str, 'program', table_dirname, table_filename, start_block, table_len_blocks)
        ...
        for table in table_list:
            ...
            for entry in table:
                ...
                if entry.erase == True:
                    add_basic_xml_attributes (xml_str, 'erase', entry.dirname, entry.filename, block_start, size_blocks)
                ...
                if len(entry.filename) > 0:
                    ...
                    add_basic_xml_attributes (xml_str, 'program', entry.dirname, entry.filename, block_start, size_blocks)
...
Example Usage:
Generate rawprogram to feed to gen_download_table.py.
...
"""
    ...
    parser.add_argument('-x', '--partitionxml', type=str, required=True, help='The partition.xml file for the Firmware Descriptor')
    ...
    fwd.from_xml_file (args.partitionxml)
