...
#
# Example:
#     python gen_part_table.py --file=FERMION.elf
...
APP_IMG_NAME     = "APP Firmware"
FS1_IMG_NAME    = "Primary Filesystem"
FS2_IMG_NAME    = "Secondary Filesystem"
UNUSED_IMG_NAME = "Unused"
RAMDUMP_IMG_NAME   = "Ramdump Partitions"
USERDATA_IMG_NAME = "Userdata Partitions"
GLANCE_IMG_NAME   = "Glance Firmware"
FLOG_IMG_NAME   = "Flash Log Partitions"
MISC0_IMG_NAME   = "MISC0 Partitions"
...
A simple example of options to create an ImageSet with an APP image,
a 16KB "reserved area" and an implicit primary file system:
    --file=$APP_IMAGE \\
    --file=ERASE --size=16KB --id=129
Note that the size of the APP image is inferred from the size
...
follwed by the APP image, the 16KB reserved space.
""")
    exit(-1)
...
def initialize_well_known_dict():
    ...
    well_known_id["APP"] = APP_IMG_ID;
    well_known_id["GLANCE"] = GLANCE_IMG_ID;
    well_known_id["FS2"] = FS2_IMG_ID;
    well_known_id["UNUSED"] = UNUSED_IMG_ID;
    well_known_id["FLOG"] = FLOG_IMG_ID;
    well_known_id["RAMDUMP"] = RAMDUMP_IMG_ID;
    well_known_id["MISC0"] = MISC0_IMG_ID;
    well_known_id["USERDATA"] = USERDATA_IMG_ID;
...
def complete_partition_list():
    ...
    for partition in partition_list:
        ...
        if ((s_file != "") and (s_file.upper() != "KEEP") and (s_file.upper() != "ERASE")):
            ...
            if (s_name == ""):
                if re.search('(?i)app', filename):
                    s_name = APP_IMG_NAME
                elif re.search('(?i)glance', filename):
                    s_name = GLANCE_IMG_NAME
        ...
        if ((s_name == "") and (s_id != "")):
            ...
            elif (s_id == RAMDUMP_IMG_ID):
                s_name = RAMDUMP_IMG_NAME
            elif (s_id == MISC0_IMG_ID):
                s_name = MISC0_IMG_NAME
            elif (s_id == USERDATA_IMG_ID):
                s_name = USERDATA_IMG_NAME
            else:
                s_name = s_id;
        ...
        if (s_file == ""):
            i_id = int(s_id)
            if ((i_id == int(FS1_IMG_ID)) or (i_id == int(FS2_IMG_ID)) or (i_id == int(UNUSED_IMG_ID)) or (i_id == int(FLOG_IMG_ID)) or (i_id == int(RAMDUMP_IMG_ID)) or (i_id == int(MISC0_IMG_ID)) or (i_id == int(USERDATA_IMG_ID))):
                s_file = "ERASE"
    ...
    if (not primary_file_system_found and default_FS1_sz != "0KB"):
...
def generate_XML_output():
    ...
    for copy in range(num_copies_of_table):
        ...
        xml_file.write("      <header rank=\"")
        xml_file.write(str(rank_set))
        xml_file.write("\" signature=\"0x54445746\" status=\"1\" version=\"")
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "o:n:i:s:f:p", [
                     ...
                     "rank=",     # rank value of the partition table
