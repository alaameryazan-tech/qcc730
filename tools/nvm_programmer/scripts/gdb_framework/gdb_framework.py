...
class GDB_Framework(object):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument('-s', '--server', default='jlink', choices=['jlink', 'ch347'], help='GDB server being used.  This should match the name of a "gdb_server_<SERVER>.py" script,  (default=jlink).')
        self.argparser.add_argument('-p', '--server-port', type=int, default=3333, help='GDB server port number,(default = 3333).')

        self.argparser.add_argument('--server-path', default=None, help='Path for JLinkGDBServerCL.exe or OpenOCD.exe')
        self.argparser.add_argument('--server-script', default=None, help='Script file for JLinkGDBServerCL.exe or OpenOCD.exe')
        self.argparser.add_argument('--serial', default=None, help='Serial number of the J-Link to use.')
        start_group = self.argparser.add_mutually_exclusive_group(required=False)
        ...
        self.argparser.add_argument('-c', '--client-exe', default='arm-none-eabi-gdb-py3', help='GDB client executable.')
        self.argparser.add_argument('-u', '--udp-port', type=int, default=19383, help='UDP port for IPC traffic to the GDB client, (default = 19383).')
    ...
    def init_gdb(self):
        ...
        '''
        if self.config['server'] == 'ch347':
            self.gdb_execute('target  extended-remote localhost:{0}'.format(self.config['server_port']))
        else:
    ...
    def load_elf(self, filename, vector_address=0x0):
        ...
        # Set stack pointer, program counter
        self.gdb_execute('set $sp=&NVM_Programmer_Stack[sizeof(NVM_Programmer_Stack)]')
        self.gdb_execute('set $pc=&NVM_Programmer_Start')
